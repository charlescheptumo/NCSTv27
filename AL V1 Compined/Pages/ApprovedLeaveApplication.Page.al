...
Page 69233 "Approved Leave Application"
{
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                {
                    ApplicationArea = Basic;
                    Caption = 'Total Annual Leave Days';
                    Editable = false;
                    Style = Strong;
                    StyleExpr = true;
                }
        ...
        area(navigation)
        {
            ...
            group("F&unctions")
            {
                ...
                action("&Post Leave Application")
                {
                    ...
                    trigger OnAction()
                    begin
                        if not Confirm('Are you sure you want to post this Leave Application?', true) then
                            Error('Leave Application not posted!');
                        //IF Status=Status:: THEN ERROR('This Leave application has already been posted');
                        AssignLeaveAllowance();
                        Rec.TestField(Rec.Posted, Rec.Posted = true);
    ...
    begin
        if (Rec.Status = Rec.Status::Released) or (Rec.Status = Rec.Status::Posted) then
    ...
    begin
        ...
        HREmp.Reset;
        HREmp.SetRange(HREmp."User ID", Rec.Tests);
    ...
    local procedure AssignLeaveAllowance()
    var
    ...
    begin
        ...
        if Rec.Status = Rec.Status::Released then begin
            ...
            if Rec."Leave Type" = 'ANNUAL' then begin
                HumanResSetup.Get();
                HumanResSetup.TestField("Leave Allowance Threshold");
                AssignMatrixX.Reset;
                AssignMatrixX.SetRange(AssignMatrixX."Employee No", Rec."Employee No");
                AssignMatrixX.SetRange(AssignMatrixX.Code, HumanResSetup."Leave Allowance Code");
                AssignMatrixX.SetRange(AssignMatrixX."Payroll Period", HumanResSetup."Leave Posting Period[FROM]", HumanResSetup."Leave Posting Period[TO]");
                if not AssignMatrixX.FindSet then begin
                    if cummdays >= HumanResSetup."Leave Allowance Threshold" then begin
                        AssignmentMatrixX1.Reset;
                        AssignmentMatrixX1.SetRange("Employee No", Rec."Employee No");
                        AssignmentMatrixX1.SetRange(AssignmentMatrixX1."Payroll Period", HumanResSetup."Leave Posting Period[FROM]", HumanResSetup."Leave Posting Period[TO]");
                        AssignmentMatrixX1.SetRange(Code, HumanResSetup."Basic Salary Code");
                        if AssignmentMatrixX1.FindLast() then begin
                            BasicAmount := AssignmentMatrixX1.Amount;
                            HumanResSetup.TestField("Leave  % of Basic allowance");
                            AllAmount := (HumanResSetup."Leave  % of Basic allowance" / 100) * BasicAmount;
                        end;
                        AssignmentMatrixX.Init;
                        AssignmentMatrixX.Code := HumanResSetup."Leave Allowance Code";
                        ...
                        AssignmentMatrixX.Amount := AllAmount;
    ...
    local procedure NotifyApplicant()
    begin
    end;
}
