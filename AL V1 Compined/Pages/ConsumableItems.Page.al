...
Page 75192 "Consumable Items"
{
        ...
        area(processing)
        {
                ...
                action(FilterByAttributes)
                {
                    ...
                    begin
                        ...
                        if ParameterCount < TypeHelper.GetMaxNumberOfParametersInSQLQuery - 100 then begin
                            ...
                            Rec.SetFilter(Rec."No.", FilterText);
                        end else begin
                            RunOnTempRec := true;
                            Rec.ClearMarks;
                            Rec.Reset;
                ...
                group(Dimensions)
                {
                    ...
                    action(DimensionsMultiple)
                    {
                        ...
                        begin
                            ...
                            DefaultDimMultiple.SetMultiRecord(Item, Rec.FieldNo(Rec."No."));
            ...
            group("Request Approval")
            {
                ...
                action(SendApprovalRequest)
                {
                    ...
                    begin
                        //cc//if ApprovalsMgmt.CheckItemApprovalsWorkflowEnabled(Rec) then
                        //cc//ApprovalsMgmt.OnSendItemForApproval(Rec);
                    end;
                }
            ...
            action(ApplyTemplate)
            {
                ...
                begin
                    CurrPage.SetSelectionFilter(Item);
                    ItemTemplate.CopyFromTemplate(ItemTemplate);
                end;
            }
            group(Display)
            {
        ...
        area(navigation)
        {
            ...
            group(Warehouse)
            {
                ...
                action("&Bin Contents")
                {
                    ...
                    Image = BinContent;
                    //cc//  RunObject = Page "Item Bin Contents";
                    //cc//  RunPageLink = "Item No." = field("No.");
                   //cc//   RunPageView = sorting("Item No.");
                   //cc//   ToolTip = 'View the quantities of the item in each bin where it exists. You can see all the important parameters relating to bin content, and you can modify certain bin content parameters in this window.';
                }
    ...
    begin
        ...
        //     //cc//SocialListeningMgt.GetItemFactboxVisibility(        Rec, SocialListeningSetupVisible, SocialListeningVisible);
     ...
     //cc//     CurrPage."Power BI Report FactBox".Page.SetCurrentListSelection(Rec."No.", false, PowerBIVisible);
        ...
        Rec.CalcFields(Rec."Location Code", Rec.Inventory);
        UserSetup.Get(UserId);
        Rec.FilterGroup(2);
        Rec.SetRange(Rec."Location Filter", UserSetup."Purchase Resp. Ctr. Filter");
        Rec.SetFilter(Rec.Inventory, '>%1', 0);
        Rec.FilterGroup(0);
    end;
    ...
    begin
       //cc//   CurrPage."Power BI Report FactBox".Page.InitFactBox(CurrPage.ObjectId(false), CurrPage.Caption, PowerBIVisible);
        Rec.CalcFields(Rec."Location Code");
    end;
