    ...
    layout
    {
        area(content)
        {
            ...
            group(Planning)
            {
                ...
                field("Training Dates"; Rec."Training Dates")
                {
                    ApplicationArea = Basic;
                }
        ...
        area(navigation)
        {
            group("&Functions")
            {
                ...
                action("&Plan")
                {
                    ...
                    trigger OnAction()
                    begin
                        if Confirm('Are you sure you want to plan for the Training Need No. ' + Rec."No." + ' ?') = true then begin
                            Rec."Planned By" := UserId;
                            Rec.Planned := true;
                            Rec."Date Planned" := Today;
                            Rec."Time Planned" := Time;
                            Message('Training need :: %1 :: has been Planned', Rec.Description);
                            Rec.Modify;
                        end;
                    end;
                }
