    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Petty Cash Voucher No"; Rec."Reference No.")
                {
                    ApplicationArea = Basic;
                    DrillDownPageID = "Posted  Petty Cash Vouchers";
                    Editable = false;
                    TableRelation = Payments."No." where("No." = field("Reference No."));

                    trigger OnDrillDown()
                    begin
                        //SETRANGE("Reference No.","No.");
                    end;
                }
    ...
    actions
    {
        ...
        area(processing)
        {
            ...
            action("Re-Open")
            {
                ...
                trigger OnAction()
                begin
                    if Confirm('Kindly reverse the entries before reopening') then begin
                        Rec."Payment Type" := Rec."payment type"::"Petty Cash Surrender";
                        Rec.Status := Rec.Status::Released;
                        Rec.Posted := true;
                        Rec.Surrendered := false;
                        Rec.Modify;
                    end;
                end;
            }
