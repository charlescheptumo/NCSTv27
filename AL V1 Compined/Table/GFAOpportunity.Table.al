...
Table 65034 "GFA Opportunity"
{
    ...
    begin
        if "No." = '' then begin
           ...
           // NoSeriesMgt.InitSeries(RMSetup."Opportunity Nos.", xRec."No. Series", 0D, "No.", "No. Series");
                "No." := NoSeriesMgt.GetNextNo(RMSetup."Opportunity Nos.", Today, true);
        end;
    ...
    begin
        if "No." <> '' then
     ;  //CC//     TempOppEntry.UpdateOppFromOpp(RecordId);
    end;
    ...
    begin
        if "No." <> '' then
       ; //CC//       TempOppEntry.CloseOppFromOpp(Rec);
    end;
    ...
    var
        ...
        SalesHeader: Record "Sales Header";
        //cc//   CustTemplate: Record "Customer Template";
        ...
        if Cont.Type = Cont.Type::Company then begin
            ...
            if ContactBusinessRelation.IsEmpty then
                if GuiAllowed then begin
                    //CC//    CustTemplateCode := Cont.ChooseCustomerTemplate;
                    if CustTemplateCode <> '' then
              //CC//         Cont.CreateCustomer(CustTemplateCode)
              //CC//  else
              //cc//   if Confirm(UpdateSalesQuoteWithCustTemplateQst) then
              //cc//if Page.RunModal(0, CustTemplate) = Action::LookupOK then
              //cc//       CustTemplateCode := CustTemplate.Code;
              //cc//   end;
              ;
                end;
            ...
            if CustTemplateCode <> '' then
           ; //CC//       SalesHeader.Validate("Sell-to Customer Template Code", CustTemplateCode);
            SalesHeader.Modify;
