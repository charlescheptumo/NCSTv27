...
Report 65004 "Grantors Detailed Aging"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Grantors Detailed Aging.rdlc';
    ApplicationArea = Basic, Suite;
    Caption = 'Grantors Detailed Aging';
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            column(Customer_Phone_No_Caption; Customer.FieldCaption(Customer."Phone No."))
            {
            }
            dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
            {
                ...
                column(Cust_Ledger_Entry_Posting_Date_; Format("Cust. Ledger Entry"."Posting Date"))
                {
                }
                column(Cust_Ledger_Entry_Document_No_; "Cust. Ledger Entry"."Document No.")
                {
                }
                column(Cust_Ledger_Entry_Description; "Cust. Ledger Entry".Description)
                {
                }
                column(Cust_Ledger_Entry_Due_Date_; Format("Cust. Ledger Entry"."Due Date"))
                {
                }
                ...
                column(Cust_Ledger_Entry_Remaining_Amount_; "Cust. Ledger Entry"."Remaining Amount")
                {
                    AutoFormatExpression = "Currency Code";
                    AutoFormatType = 1;
                }
                column(Cust_Ledger_Entry_Currency_Code_; "Cust. Ledger Entry"."Currency Code")
                {
                }
                column(Cust_Ledger_Entry_Remaining_Amt_LCY_; "Cust. Ledger Entry"."Remaining Amt. (LCY)")
                {
                    AutoFormatType = 1;
                }
                ...
                begin
                    if "Cust. Ledger Entry"."Due Date" = 0D then
                        OverDueMonths := 0
                    else begin
                        OverDueMonths :=
                          (Date2dmy(EndDate, 3) - Date2dmy("Cust. Ledger Entry"."Due Date", 3)) * 12 +
                          Date2dmy(EndDate, 2) - Date2dmy("Cust. Ledger Entry"."Due Date", 2);
                        if Date2dmy(EndDate, 1) < Date2dmy("Cust. Ledger Entry"."Due Date", 1) then
                            OverDueMonths := OverDueMonths - 1;
                    end;
                    "Cust. Ledger Entry".SetRange("Cust. Ledger Entry"."Date Filter", 0D, EndDate);
                    "Cust. Ledger Entry".CalcFields("Cust. Ledger Entry"."Remaining Amount", "Cust. Ledger Entry"."Remaining Amt. (LCY)");
                    if "Cust. Ledger Entry"."Remaining Amount" = 0 then
                        CurrReport.Skip;
                    CurrencyTotalBuffer.UpdateTotal(
                      "Cust. Ledger Entry"."Currency Code", "Cust. Ledger Entry"."Remaining Amount", "Cust. Ledger Entry"."Remaining Amt. (LCY)", Counter);
                end;
                ...
                begin
                    if OnlyOpen then begin
                        ...
                        "Cust. Ledger Entry".SetRange("Cust. Ledger Entry"."Due Date", 0D, EndDate);
                    end else
                        "Cust. Ledger Entry".SetRange("Cust. Ledger Entry"."Due Date", 0D, EndDate);
                    Counter := 0;
                end;
